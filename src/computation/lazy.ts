 ...
 */
export const Lazy = <T>(callback: LazyValueCallback<T>): Lazy<T> => callback satisfies Lazy<T>;
 ...
 * @param callback - the callback that computes a value.
 */
export const AsyncLazy = <T>(callback: LazyValueCallback<Promise<T>>) => callback;
 ...
 */
export const computed = <T>(value: Lazy<T>): value is T => typeof value !== 'function';
 ...
 */
export const compute = <T>(value: Lazy<T>) => computed(value) ? value : value() satisfies Lazy<T>;